...
const LOGIN_REQUUIRED_URL = "/login";
...
export function AuthProvider({ children }) {
  ...
  const loginRequiredRedirect = () => {
    ...
    setIsAuthenticated(false);
    localStorage.setItem(LOCAL_STORAGE_KEY, "0");
    const loginWithNextUrl = `${LOGIN_REQUUIRED_URL}?next=${pathName}`;
    if (LOGIN_REQUUIRED_URL === pathName) {
      loginWithNextUrl = `${LOGIN_REQUUIRED_URL}`;
    }
